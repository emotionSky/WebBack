<template>
	<!-- 使用el-container容器进行布局 -->
	<el-container>
		<!-- el-header占据整个页面，设置高度，使用padding控制元素所有四条边的内边距区域。 -->
		<el-header height="80px" style="padding:0">
			<div class="header">教务系统学生管理</div>
		</el-header>
		<el-container>
			<!-- el-aside占据整个高度，可设置宽度 -->
			<el-aside width="200px">
				<!-- 使用el-menu来实现导航栏 unique-opened保证只有一个子菜单展开 -->
				<el-menu class="aside" @select="selectFunc" default-active="1" :unique-opened="true">
					<el-sub-menu index="1">
						<template #title>
							<!-- span是短语内容的通用行内容器，并没有任何特殊语义。 -->
							<span>七年级</span>
						</template>
			...
			<el-container>
				<!-- 使用margin为给定元素设置所有四个（上右下左）方向的外边距属性。 -->
